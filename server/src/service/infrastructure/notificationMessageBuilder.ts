...
export class NotificationMessageBuilder implements INotificationMessageBuilder {
	...
	private buildEscalationContent(monitor: Monitor, monitorStatusResponse: MonitorStatusResponse): NotificationContent {
		...
		const summary = `ESCALATION ALERT: Monitor "${monitor.name}" has been down for an extended period and requires immediate attention.`;
		const details = [
			`URL: ${monitor.url}`,
			`Status: Down (Escalation triggered)`,
			`Type: ${monitor.type}`,
		];
